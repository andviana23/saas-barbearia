...
async function executeSQLDirect(sql) {
  try {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      console.log(
        `   ⚠️  Comando ${i + 1}/${commands.length}: ${command.substring(0, 60)}...`
      )
      console.log(
        `   💡 Este comando precisa ser executado manualmente no SQL Editor do Supabase`
      )
    }
...
async function runMigration(migration) {
  ...
  try {
    ...
    console.error(
      `   ❌ Erro ao processar migração ${migration.version}: ${error.message}`
    )
    return { success: false, error: error.message }
  }
}
...
async function showStatus() {
  ...
  files.forEach((file) => {
    ...
    const name =
      file.name.length > 32 ? file.name.substring(0, 29) + '...' : file.name
...
async function migrate() {
  ...
  console.log(
    '⚠️  IMPORTANTE: Como o Supabase não permite execução de DDL via API,'
  )
  console.log(
    '   as migrações precisam ser executadas manualmente no SQL Editor.\n'
  )
