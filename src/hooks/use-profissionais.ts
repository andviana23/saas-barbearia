...
import type {
  ProfissionalFilterData,
  HorarioProfissionalFilterData,
} from '@/schemas'
...
export const DISPONIBILIDADE_PROFISSIONAL_QUERY_KEY =
...
export function useDisponibilidadeProfissional(
  profissionalId: string,
  data: string
) {
  return useQuery({
...
export function useProfissionaisComHorarios(unidadeId?: string) {
      ...
      const profissionaisComHorarios = await Promise.all(
        (profissionaisQuery.data.data as any).profissionais.map(
          async (profissional: { id: string; [key: string]: unknown }) => {
            const horariosResult = await listHorariosProfissional(
              profissional.id,
              { page: 1, limit: 20, order: 'asc', ativo: true }
            )
            return {
              ...profissional,
  ...
  return {
    ...
    isLoading:
      profissionaisQuery.isLoading || profissionaisComHorarios.isLoading,
