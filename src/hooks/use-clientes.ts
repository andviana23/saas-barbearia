...
export function useBuscarClientePorTelefone(telefone: string, unidadeId: string) {
  return useQuery({
...
export function useBuscaInteligentClientes(searchTerm: string, unidadeId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (isPhone && searchTerm.length >= 10) {
        const telefoneResult = await buscarClientePorTelefone(searchTerm, unidadeId || '');
