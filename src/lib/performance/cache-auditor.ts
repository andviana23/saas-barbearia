...
class CacheAuditor {
  ...
  analyzeCacheOptimizations(): CacheOptimization[] {
    ...
    for (const [queryKey, times] of this.queryTimes.entries()) {
      ...
      if (
        Math.abs(currentStaleTime - recommendedStaleTime) >
        recommendedStaleTime * 0.5
      ) {
        optimizations.push({
          queryKey,
          currentStaleTime,
  ...
  generatePerformanceReport(): {
    ...
    const totalCacheHits = Array.from(this.cacheHits.values()).reduce(
      (sum, hits) => sum + hits,
      0
    )
  ...
  private generateRecommendations(
    ...
    const lowHitRateQueries = optimizations.filter((o) =>
      o.reason.includes('Baixo hit rate')
    )
    if (lowHitRateQueries.length > 0) {
      recommendations.push(
        'Considere aumentar o staleTime para queries com baixo hit rate.'
      )
    }

    const frequentQueries = optimizations.filter((o) =>
      o.reason.includes('muito frequentemente')
    )
    if (frequentQueries.length > 0) {
      recommendations.push(
        'Implemente debouncing para queries executadas muito frequentemente.'
      )
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Cache está otimizado. Continue monitorando para manter a performance.'
      )
    }
  ...
  private getRecommendedStaleTime(queryKey: string): number {
    ...
    return this.recommendedStaleTimes[category] || 300000
  }
...
export function useCacheAuditor() {
  return {
    ...
    analyzeCacheOptimizations:
      cacheAuditor.analyzeCacheOptimizations.bind(cacheAuditor),
    generatePerformanceReport:
      cacheAuditor.generatePerformanceReport.bind(cacheAuditor),
