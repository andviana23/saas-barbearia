...
export class PreloadManager {
  ...
  preloadInBackground(componentName: string, importFn: () => Promise<any>): void {
    if (this.preloadedComponents.has(componentName)) {
      return;
    }
...
export function usePreload() {
  return {
    ...
    preloadInBackground: preloadManager.preloadInBackground.bind(preloadManager),
...
export function analyzeBundleSize(): void {
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log('Preloaded Components:', Array.from(preloadManager['preloadedComponents']));
    console.groupEnd();
  }
}
