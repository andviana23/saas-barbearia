...
>(
  ...
  options: {
    ...
    mutationOptions?: Partial<
      UseMutationOptions<ActionResult<TData>, Error, TVariables>
    >
  } = {}
) {
...
export function useDeleteMutation<TVariables = MutationVariables>(
  ...
  return useStandardMutation(mutationFn, {
    ...
    onSuccess: (result, variables) => {
      ...
      if (result.success && getId) {
        const id = getId(variables)
        const entityKeys = invalidatePatterns as Record<
          string,
          { detail: (id: string) => readonly unknown[] }
        >
        if (entityKeys[entity]?.detail) {
          queryClient.removeQueries({
            queryKey: entityKeys[entity].detail(id),
          })
        }
      }
...
export const optimisticUpdates = {
  ...
  updateInList<T>(
    ...
    queryClient.setQueryData(queryKey, (oldData: ListData<T> | undefined) => {
...
export const mutationStates = {
  isLoading: (mutation: UseMutationResult<ActionResult, Error, unknown>) =>
    mutation.isPending,
  isError: (mutation: UseMutationResult<ActionResult, Error, unknown>) =>
    mutation.isError,
  isSuccess: (mutation: UseMutationResult<ActionResult, Error, unknown>) =>
    mutation.isSuccess && mutation.data?.success,
  hasValidationErrors: (
    mutation: UseMutationResult<ActionResult, Error, unknown>
  ) =>
    mutation.data && !mutation.data.success && mutation.data.errors?.length > 0,
