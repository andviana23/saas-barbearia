...
export async function deleteProduto(id: string): Promise<ActionResult> {
  try {
    ...
    const { data: produto, error: fetchError } = await supabaseServer
      .from('produtos')
      .select('id, nome')
      .eq('id', id)
      .single()
    ...
    const { data: vendasItens, error: vendasError } = await supabaseServer
      .from('vendas_itens')
      .select('id')
      .eq('produto_id', id)
      .limit(1)
...
export async function baixarEstoqueProduto(
  ...
  try {
    ...
    const { data: produtoAtualizado, error } = await supabaseServer
      .from('produtos')
      .update({ estoque: novoEstoque })
      .eq('id', id)
      .select()
