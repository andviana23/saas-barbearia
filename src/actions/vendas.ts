...
export async function createVenda(formData: FormData): Promise<ActionResult> {
  ...
  return withValidation(CreateVendaSchema, data, async (validatedData) => {
    const { data: venda, error } = await supabaseServer
      .from('vendas')
...
export async function addItemVenda(formData: FormData): Promise<ActionResult> {
  ...
  return withValidation(CreateVendaItemSchema, data, async (validatedData) => {
    try {
      ...
      const { data: venda, error: vendaError } = await supabaseServer
        .from('vendas')
        .select('id, status')
      ...
      if (venda.status !== 'aberta') {
        throw new Error(
          'Não é possível adicionar itens a uma venda que não está aberta'
        )
      }

      // Verificar se o produto existe
      const { data: produto, error: produtoError } = await supabaseServer
        .from('produtos')
      ...
      if (!estoqueResult.success) {
        ...
        await supabaseServer.from('vendas_itens').delete().eq('id', item.id)
...
export async function removeItemVenda(itemId: string): Promise<ActionResult> {
  try {
    ...
    if (item.venda?.status !== 'aberta') {
      throw new Error(
        'Não é possível remover itens de uma venda que não está aberta'
      )
    }
...
export async function finalizarVenda(vendaId: string): Promise<ActionResult> {
  try {
    ...
    const { data: vendaFinalizada, error: updateError } = await supabaseServer
      .from('vendas')
      .update({ status: 'paga' })
      .eq('id', vendaId)
...
export async function cancelarVenda(vendaId: string): Promise<ActionResult> {
  try {
    ...
    const { data: vendaCancelada, error: updateError } = await supabaseServer
      .from('vendas')
...
async function recalcularTotalVenda(vendaId: string) {
  const { data: itens, error } = await supabaseServer
    .from('vendas_itens')
    .select('subtotal')
    .eq('venda_id', vendaId)
  ...
  if (!error && itens) {
    const total = itens.reduce(
      (acc: number, item: { subtotal: number | string }) =>
