 ...
 */
export async function createMovimentacao(formData: FormData): Promise<ActionResult> {
  const data = {
  ...
  return withValidation(CreateMovimentacaoSchema, data, async (validatedData) => {
 ...
 */
export async function updateMovimentacao(formData: FormData): Promise<ActionResult> {
  ...
  return withValidation(UpdateMovimentacaoSchema, data, async (validatedData) => {
    const supabase = createServerSupabase();
...
export async function listMovimentacoes(
  ...
  return withValidation(MovimentacaoFilterSchema, params, async (filters) => {
    ...
    if (filters.busca) {
      query = query.or(`descricao.ilike.%${filters.busca}%,categoria.ilike.%${filters.busca}%`);
    }
...
export async function deleteMovimentacao(id: string): Promise<ActionResult> {
  ...
  try {
    ...
    const { error } = await supabase.from('financeiro_mov').delete().eq('id', id);
 ...
 * Lançamento rápido
 */
export async function lancamentoRapido(formData: FormData): Promise<ActionResult> {
  const data = {
    unidade_id: formData.get('unidade_id'),
    tipo: formData.get('tipo'),
    valor: Number(formData.get('valor')),
 ...
 * Calcular comissão
 */
export async function calcularComissao(formData: FormData): Promise<ActionResult> {
  const data = {
 ...
 * Fechamento de caixa
 */
export async function fechamentoCaixa(formData: FormData): Promise<ActionResult> {
  const data = {
    unidade_id: formData.get('unidade_id'),
  ...
  return withValidation(FechamentoCaixaSchema, data, async (validatedData) => {
    ...
    movimentacoes?.forEach((mov: { tipo: string; valor: number; meio_pagamento?: string }) => {
      const valor = mov.tipo === 'entrada' ? mov.valor : -mov.valor;
      if (mov.meio_pagamento && totaisSistema.hasOwnProperty(mov.meio_pagamento)) {
        totaisSistema[mov.meio_pagamento as keyof typeof totaisSistema] += valor;
      }
    });
    ...
    const totalSistema = Object.values(totaisSistema).reduce((sum, val) => sum + val, 0);
...
export async function getEstatisticasFinanceiras(
  ...
  return withValidation(EstatisticasFinanceirasSchema, params, async (filtros) => {
    const supabase = createServerSupabase();
...
function processarResumoIiario(
  ...
  movimentacoes.forEach((mov) => {
    ...
    resumoPorDia[data].saldo = resumoPorDia[data].entradas - resumoPorDia[data].saidas;
  });
...
function processarResumoMensal(
  ...
  movimentacoes.forEach((mov) => {
    ...
    resumoPorMes[mes].saldo = resumoPorMes[mes].entradas - resumoPorMes[mes].saidas;
  });
...
function processarPorProfissional(
  ...
  movimentacoes.forEach((mov) => {
    ...
    resumoPorProfissional[profId].saldo =
      resumoPorProfissional[profId].entradas - resumoPorProfissional[profId].saidas;
  });
...
function calcularEstatisticas(
  ...
  return {
    ...
    ticket_medio: movimentacoes.length > 0 ? totalEntradas / movimentacoes.length : 0,
