...
// Implementação original de agendamentos extraída de agendamentos.ts para permitir
// instrumentação de cobertura (wrapper com 'use server' permanece em agendamentos.ts)
...
export interface Appointment {
  ...
  profissional?: { id: string; nome: string; telefone: string };
...
export interface AppointmentServico {
  ...
  servico?: { id: string; nome: string; descricao?: string };
}

export interface DisponibilidadeInfo {
  data: string;
  horarios_disponiveis: HorarioDisponivelData[];
  horarios_ocupados: Array<{ inicio: string; fim: string; cliente_nome: string }>;
}
...
async function createAppointmentAction(
  ...
  try {
    ...
    const total = data.servicos.reduce((acc, s) => acc + s.preco_aplicado, 0);
    const { data: appointment, error: appointmentError } = await supabase
      ...
      .single();
    if (appointmentError) {
      return { success: false, error: `Erro ao criar agendamento: ${appointmentError.message}` };
    }
    const servicosData = data.servicos.map((s) => ({
      appointment_id: appointment.id,
      servico_id: s.servico_id,
      preco_aplicado: s.preco_aplicado,
      duracao_aplicada: s.duracao_aplicada,
    }));
    ...
    return { success: false, error: `Erro inesperado ao criar agendamento: ${error}` };
  }
}
...
async function rescheduleAppointmentAction(
  ...
  try {
    ...
    const { data: appointment, error: fetchError } = await supabase
      .from('appointments')
      .select(`*, appointments_servicos(duracao_aplicada)`)
      .eq('id', data.id)
      .single();
    if (fetchError || !appointment) return { success: false, error: 'Agendamento não encontrado.' };
    ...
    const duracao_total = appointment.appointments_servicos.reduce(
      (total: number, s: { duracao_aplicada: number }) => total + s.duracao_aplicada,
      0,
    );
    ...
    const updateData: { inicio: string; fim: string; profissional_id?: string; notas?: string } = {
      inicio: data.novo_inicio.toISOString(),
      fim: novo_fim.toISOString(),
    };
    if (data.profissional_id) updateData.profissional_id = data.profissional_id;
    ...
    const { data: updatedAppointment, error: updateError } = await supabase
      ...
      .select()
      .single();
    if (updateError) return { success: false, error: `Erro ao reagendar: ${updateError.message}` };
    return { success: true, data: updatedAppointment as Appointment };
  } catch (error) {
    return { success: false, error: `Erro inesperado ao reagendar: ${error}` };
  }
}
...
async function updateAppointmentStatusAction(
  ...
  try {
    ...
    const updateData: { status: AppointmentStatus; notas?: string } = { status: data.status };
    ...
    return { success: false, error: `Erro inesperado ao atualizar status: ${error}` };
  }
}
...
async function cancelAppointmentAction(
  ...
  try {
    ...
    if (fetchError || !appointment) return { success: false, error: 'Agendamento não encontrado.' };
    if (['concluido', 'cancelado'].includes(appointment.status)) {
      return { success: false, error: 'Agendamento já foi concluído ou cancelado.' };
    }
    const agora = new Date();
    const inicioDate = new Date(appointment.inicio);
    const antecedencia = Math.round((inicioDate.getTime() - agora.getTime()) / (1000 * 60));
    ...
    const { data: cancelledAppointment, error: cancelError } = await supabase
      .from('appointments')
      .update({ status: 'cancelado', notas: notasCompletas })
      .eq('id', data.id)
      .select()
      .single();
    if (cancelError)
      return { success: false, error: `Erro ao cancelar agendamento: ${cancelError.message}` };
    return { success: true, data: cancelledAppointment as Appointment };
  } catch (error) {
    return { success: false, error: `Erro inesperado ao cancelar agendamento: ${error}` };
  }
}
...
async function listAppointmentsAction(
  filters: AppointmentFilterData,
): Promise<
  ActionResult<{ appointments: Appointment[]; total: number; page: number; limit: number }>
> {
  try {
    const supabase = createServerSupabase();
    let query = supabase
      .from('appointments')
      .select(
        `*, cliente:clientes(id, nome, telefone, email), profissional:profissionais(id, nome, telefone), appointments_servicos( id, servico_id, preco_aplicado, duracao_aplicada, servico:servicos(id, nome, descricao) )`,
      );
    ...
    if (filters.data_inicio) query = query.gte('inicio', filters.data_inicio.toISOString());
    if (filters.data_fim) query = query.lte('inicio', filters.data_fim.toISOString());
    if (filters.busca) {
    ...
    if (error) return { success: false, error: `Erro ao buscar agendamentos: ${error.message}` };
    return {
      success: true,
      data: {
    ...
    return { success: false, error: `Erro inesperado ao listar agendamentos: ${error}` };
  }
}
...
async function checkDisponibilidadeAction(
  ...
  try {
    ...
    let query = supabase
      .from('appointments')
      .select(`inicio, fim, cliente:clientes(nome)`)
      .eq('profissional_id', data.profissional_id)
      .gte('inicio', inicioHorario.toISOString())
      .lt('inicio', fimHorario.toISOString())
      .not('status', 'in', '(cancelado,faltou)')
      .order('inicio');
    if (data.agendamento_id) query = query.neq('id', data.agendamento_id);
    const { data: appointments, error } = await query;
    if (error)
      return { success: false, error: `Erro ao verificar disponibilidade: ${error.message}` };
    ...
    for (let hora = 6; hora < 22; hora++) {
      for (let minuto = 0; minuto < 60; minuto += 15) {
        ...
        const hasConflict = horariosOcupados.some((ocupado) => {
    ...
    return { success: false, error: `Erro inesperado ao verificar disponibilidade: ${error}` };
  }
}
...
async function getAgendamentoStatsAction(
  ...
  try {
    ...
    let query = supabase
      .from('appointments')
      .select(
        `id, status, total, inicio, appointments_servicos( servico:servicos(id, nome), preco_aplicado )`,
      )
    ...
    if (filters.profissional_id) query = query.eq('profissional_id', filters.profissional_id);
    const { data: appointments, error } = await query;
    if (error) return { success: false, error: `Erro ao buscar estatísticas: ${error.message}` };
    const dados = appointments || [];
    const totalAgendamentos = dados.length;
    const agendamentosConfirmados = dados.filter((a: any) =>
      ['confirmado', 'em_atendimento'].includes(a.status),
    ).length;
    const agendamentosConcluidos = dados.filter((a: any) => a.status === 'concluido').length;
    const agendamentosCancelados = dados.filter((a: any) =>
      ['cancelado', 'faltou'].includes(a.status),
    ).length;
    const receitaTotal = dados
      .filter((a: any) => a.status === 'concluido')
      .reduce((total: number, apt: any) => total + (apt.total || 0), 0);
    ...
    const horasDisponiveis = diasNoPeriodo * 8 * 60;
    const minutosOcupados = dados
      .filter((a: any) => !['cancelado', 'faltou'].includes(a.status))
    ...
    const servicosMap = new Map<
      string,
      { servico_id: string; nome: string; quantidade: number; receita_total: number }
    >();
    dados.forEach((apt: any) => {
      apt.appointments_servicos?.forEach((as: any) => {
        ...
        if (servicoId) {
          const existing = servicosMap.get(servicoId) || { quantidade: 0, receita_total: 0 };
    ...
    return { success: false, error: `Erro inesperado ao calcular estatísticas: ${error}` };
  }
}
...
export async function getAppointmentById(id: string): Promise<ActionResult<Appointment>> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('id', id)
      .single();
    if (error) return { success: false, error: `Agendamento não encontrado: ${error.message}` };
    return { success: true, data: data as Appointment };
  } catch (error) {
    return { success: false, error: `Erro inesperado ao buscar agendamento: ${error}` };
  }
}
